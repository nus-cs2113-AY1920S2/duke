...
Let's return to duke.Duke.duke.Duke and convert it to use FXML instead.
...
As part of the effort to separate the code handling duke.Duke.duke.Duke's logic and UI, let's _refactor_ the UI-related code to its own class.
...
**MainWindow.java**
```java
import duke.Duke.duke.Duke;import duke.Duke;import javafx.fxml.FXML;
import javafx.scene.control.Button;
import javafx.scene.control.ScrollPane;
import javafx.scene.control.TextField;
...
Let's create a new `Main` class as the bridge between the existing logic in `duke.Duke.duke.Duke` and the UI in `MainWindow`.
...
/**
 * A GUI for duke.Duke.duke.Duke using FXML.
 */
