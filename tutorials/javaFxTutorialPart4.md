...
Let's return to Jan and convert it to use FXML instead.
...
As part of the effort to separate the code handling Jan's logic and UI, let's _refactor_ the UI-related code to its own class.
...
Let's create a new `Main` class as the bridge between the existing logic in `Jan` and the UI in `MainWindow`.
...
/**
 * A GUI for Jan using FXML.
 */
public class Main extends Application {

    private Jan jan = new Jan();
