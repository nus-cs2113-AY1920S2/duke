...
Let's return to src.main.java.Duke and convert it to use FXML instead.
...
We will get to that later.
   ...
   ![command.Controller for MainWindow](assets/MainWindowController.png)
...
As part of the effort to separate the code handling src.main.java.Duke's logic and UI, let's _refactor_ the UI-related code to its own class.
...
import javafx.scene.layout.AnchorPane;
import javafx.scene.layout.VBox;import src.main.java.Duke;
/**
 * command.Controller for MainWindow. Provides the layout for the other controls.
 */
...
Let's create a new `Main` class as the bridge between the existing logic in `src.main.java.Duke` and the UI in `MainWindow`.
...
/**
 * A GUI for src.main.java.Duke using FXML.
 */
