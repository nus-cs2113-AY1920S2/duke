...
As part of the effort to separate the code handling duke.duke's logic and ui.ui, let's _refactor_ the ui.ui-related code to its own class.
...
Let's create a new `Main` class as the bridge between the existing logic in `duke.duke` and the ui.ui in `MainWindow`.
